...
def test_langfuse_chat_context_picks_user_email(
    mock_st: MagicMock,
    ...
    ctx = streamlit_app._langfuse_chat_context(get_settings())
    assert ctx.user_id == "a@b.co"


def test_langfuse_chat_context_falls_back_to_id(
    mock_st: MagicMock,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("GOOGLE_CLOUD_PROJECT", "p")
...
def test_langfuse_chat_context_no_user(mock_st: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GOOGLE_CLOUD_PROJECT", "p")
    get_settings.cache_clear()
    mock_st.user = None
    ctx = streamlit_app._langfuse_chat_context(get_settings())
